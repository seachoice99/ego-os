...
### Local control server (`automation/control_server.js`) — pure JSON API, no HTML

A dependency-free HTTP server (Node's built-in `http` only) bound to `127.0.0.1` exclusively, with defense in depth rejecting any request whose socket address isn't loopback. It never re-implements the engine's own logic — task loading, process management, and state rules are all required from `claude_task_runner.js`/`runner_control.js`; commands are issued by writing to the same file-based control protocol the engine already polls. Routes: `GET /api/status|tasks|tasks/:id|events|logs|usage`, `POST /api/runner/{start,pause,resume,stop-after-stage,emergency-stop}`, `POST /api/tasks/:id/{hold,unhold,retry,skip}`, `POST /api/tasks/reorder`. Task ids and log filenames are validated and containment-checked against path traversal; request bodies are capped at 64KB and rejected with a real `413` (not a raw connection reset — `req.destroy()` on an oversized body was found live to kill the socket before the `413` could be flushed, and is deliberately not called); `emergency-stop`/`skip`/`retry` require an explicit `{confirm: true}` body; exactly one control server per workspace via its own lock file (`ego-os-control-server.lock`, next to the runner's own).

**No HTML/CSS/JS is served from here.** In production this server runs co-located with Ego OS itself, as its own systemd unit (`ego-os-runner.service`, a separate git checkout under `/opt/ego-os-runner/ego-os`, distinct from `/opt/ego-os`'s `ego-os.service` — see `DEPLOYMENT.md`) — the Owner's browser can never reach `127.0.0.1:4756` on that machine directly, so a standalone dashboard here was unreachable in practice and has been removed. The one real UI is Ego OS's own Owner-authenticated `/automation` page (`ego_os/templates/automation.html`, via `ego_os/automation_bridge.py` calling this same JSON API on the same host) — casual project cards, drag-and-drop priority, and the Claude/Codex usage panel all live there now (see that page/`automation_bridge.py` for details, not here).

**Launch:** `npm run control-server` (starts only the API; `npm run runner` starts the runner engine). Both are plain `node` invocations — no build step, no Docker.

## Casual grouped queue view + Claude/Codex usage tracker (rendered in Ego OS's own `/automation` page)
...
- **Casual project cards** (`automation/project_groups.js`, consumed by `runner_control.summarizeTask()` so every `GET /api/tasks` row already carries `group_key`/`group_name`/`group_casual_summary`): a pure, id-prefix-based lookup (never a new required task field) groups tasks into a named, casually-described card — e.g. all `MED-*` tasks collapse into "Клод + Кодекс работают вместе". `ego_os/main.py`'s `automation_page()` groups the already-tagged task list in plain Python (no client-side grouping logic duplicated). Each card is collapsed by default, shows a plain-language summary and a "done/total" count, and flags itself only when a task inside genuinely needs the Owner's attention (`blocked`/`waiting_for_auth`/`failed`/`interrupted`).
